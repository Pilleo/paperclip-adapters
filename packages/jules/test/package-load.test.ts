...
beforeAll(() => {
  ...
  describe.skip('Package Load Test', () => {
    let tgzPath: string;
    let extractDir: string;

    beforeAll(() => {
        // Run npm pack
        const output = execSync('npm pack', { encoding: 'utf-8', cwd: path.resolve(process.cwd(), 'packages/jules') }).trim();
        const tarballName = output.split('\n').pop()!;
        tgzPath = path.resolve(process.cwd(), 'packages/jules', tarballName);
        ...
        execSync(`tar -xzf ${tgzPath} -C ${extractDir}`, { cwd: path.resolve(process.cwd(), 'packages/jules') });
    }, 30000);
